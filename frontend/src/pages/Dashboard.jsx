  ...
  return (
    <div>
      ...
      <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-6">
        <h2 className="text-lg font-semibold text-[var(--text-primary)]">Ventas por hora — jornada actual</h2>
        <div className="mt-4">
...
function SeccionJornada({ jornada, ventasJornada, onAbrir, onCerrar }) {
  if (!jornada) {
    return (
      <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-6">
        <h2 className="text-lg font-semibold text-[var(--text-primary)]">Jornada</h2>
        <p className="mt-1 text-sm text-[var(--text-secondary)]">El día no ha comenzado. Abre la jornada para empezar.</p>
        <button
          type="button"
          onClick={onAbrir}
    ...
    <div className="mt-6 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">{ventasJornada?.cantidad_pedidos || 0} pedidos cobrados</p>
        </div>
        <button
          type="button"
          onClick={onCerrar}
          className="rounded-lg border border-[var(--accent)] px-4 py-2 text-sm font-semibold text-[var(--accent)] hover:bg-[var(--accent)]/10"
        >
          Cerrar jornada
        </button>
      </div>
...
function TarjetaResumen({ icono: Icono, titulo, valor, subtitulo, link }) {
  const contenido = (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5 transition-colors hover:border-[var(--accent)]/50">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[var(--text-secondary)]">{titulo}</p>
        {Icono && <Icono size={16} className="text-[var(--text-secondary)]" />}
      </div>
      <p className="mt-2 truncate text-2xl font-bold text-[var(--text-primary)]">{valor}</p>
      {subtitulo && <p className="mt-1 text-xs text-[var(--text-secondary)]">{subtitulo}</p>}
    </div>
  );
...
function TarjetaVentasHoy({ resumen }) {
  if (!resumen.jornada_abierta) {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
        <p className="text-sm text-[var(--text-secondary)]">Ventas esta jornada</p>
        <p className="mt-2 text-lg font-semibold text-[var(--text-secondary)]">Sin jornada abierta</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5">
      <p className="text-sm text-[var(--text-secondary)]">Ventas esta jornada</p>
      <p className="mt-2 text-2xl font-bold text-[var(--text-primary)]">{formatearPrecio(resumen.ventas_jornada)}</p>
      {resumen.hay_jornada_anterior && (
        <div
          className={`mt-1 flex items-center gap-1 text-xs font-medium ${subio ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}
        >
...
function ModalCerrarJornada({ ventasJornada, cargando, onGuardar, onCancelar }) {
  ...
  return (
    <Modal titulo="Cerrar jornada" onClose={onCancelar}>
      <div className="space-y-3 rounded-lg bg-[var(--bg-secondary)] p-4 text-sm">
        <div className="flex justify-between">
          <span className="text-[var(--text-secondary)]">Total vendido</span>
          <span className="font-semibold text-[var(--text-primary)]">{formatearPrecio(ventasJornada?.total_ventas || 0)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[var(--text-secondary)]">Cantidad de pedidos</span>
          <span className="font-semibold text-[var(--text-primary)]">{ventasJornada?.cantidad_pedidos || 0}</span>
        </div>
        <div className="border-t border-[var(--border)] pt-3">
          <p className="mb-2 text-xs uppercase tracking-wide text-[var(--text-secondary)]">Desglose por método de pago</p>
          ...
          {(ventasJornada?.por_metodo_pago || []).length === 0 && (
            <p className="text-[var(--text-secondary)]">Sin ventas todavía</p>
          )}
        </div>
        <div className="flex justify-between border-t border-[var(--border)] pt-3">
          <span className="text-[var(--text-secondary)]">Efectivo esperado en caja</span>
          <span className="font-semibold text-[var(--text-primary)]">{formatearPrecio(montoEsperado)}</span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        ...
        {diferencia !== null && (
          <p className={`text-sm font-semibold ${diferencia === 0 ? 'text-[var(--success)]' : diferencia > 0 ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}>
            {diferencia === 0
